...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/oa/oaNotify/course?status=${status}">科目列表</a></li>
		<li class="active"><a href="${ctx}/oa/oaNotify/form2?id=${course.id}&&status=${status}">科目<shiro:hasPermission name="oa:oaNotify:edit">${course.status eq '1' ? '查看' : not empty course.id ? '修改' : '添加'}</shiro:hasPermission><shiro:lacksPermission name="oa:oaNotify:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="course" action="${ctx}/oa/oaNotify/save2?status=${status}" method="post" class="form-horizontal">
